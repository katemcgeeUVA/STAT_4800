...
run_down <- function(down, ytg, fp, max_yrds){
  # simulate the down
  ytg <- max(ytg - max_yrds, 0)  # update YTG
...
down_one <- function(down, ytg, fp){
  ...
  max_yrds <- sample(0:5, 1)  # yards gained on first down
  run_down(down, ytg, fp, max_yrds)
}

down_two <- function(down, ytg, fp){
  # simulate the second down
  max_yrds <- sample(0:6, 1)  # yards gained on second down
  run_down(down, ytg, fp, max_yrds)
}

down_three <- function(down, ytg, fp){
  # simulate the third down
  max_yrds <- sample(0:7, 1)  # yards gained on third down
  run_down(down, ytg, fp, max_yrds)
}
